...
1. В `App.tsx` заведи массив карточек типа `Card[]` (тип уже готов в `client/src/types/card.ts`): 3–4 карточки, хотя бы одна с `isDone: true`. Передай массив в `BoardColumn` через props.
...
Теперь заменим захардкоженный массив настоящими данными. Всё подготовлено заранее: мок-сервер запускается командой `npm run server` во **втором** терминале (первый занят `npm run dev`; обе команды — из папки `client/`), а функции `fetchCards` и `createCard` уже лежат в `client/src/api/cards.ts` — писать `fetch` руками не нужно. Образцы кода — в лекции, разделы «Запросы: useQuery» и «Мутации: useMutation»; что такое мок-сервер — раздел «Необходимо знать для дальнейшего».
...
- [ ] при открытии страницы на мгновение видна «Загрузка…», затем появляются карточки с сервера (те, что в `client/db.json`);
...
> **Критерии приёмки:**
>
> 1. В `client/src/store/` создан стор Zustand с полем `hideDone` (по умолчанию `false`) и экшеном `toggleHideDone`;
...
- Merge request по ошибке открыт в репозиторий курса, а не в свою копию (ловушка форка на GitHub) — проверяй при создании, в какой репозиторий он вливается;
